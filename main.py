...
from astrbot.api.event import filter, AstrMessageEvent, MessageEvent
...
class RandomWifePlugin(Star):
    ...
    @filter.message_text("今日老婆", alias={'抽老婆'})
    async def draw_wife_with_at(self, event: AstrMessageEvent):
        """抽取今日老婆（带@功能），别名"抽老婆" - 文本版本"""
        async for result in self._draw_wife_common(event, with_at=True):
            yield result
    
    @filter.command("抽老婆-@",alias={'今日老婆-@'})
    async def draw_wife_without_at_command(self, event: AstrMessageEvent):
        """抽取今日老婆（不带@功能），别名"今日老婆-@" - @版本"""
        async for result in self._draw_wife_common(event, with_at=False):
            yield result
    
    @filter.message_text("抽老婆-@",alias={'今日老婆-@'})
    async def draw_wife_without_at(self, event: AstrMessageEvent):
        """抽取今日老婆（不带@功能），别名"今日老婆-@" - 文本版本"""
        async for result in self._draw_wife_common(event, with_at=False):
            yield result
    ...
    @filter.regex(r'^(今天)?抽(一)?个老婆$', regex_flags=0)
    async def draw_wife_today_regex(self, event: AstrMessageEvent):
        """今天抽老婆 - 正则匹配"""
        async for result in self._draw_wife_common(event, with_at=True):
            yield result
    ...
    async def _show_my_wives_common(self, event: AstrMessageEvent):
        """显示用户的抽取历史的共用方法"""
        if event.is_private_chat():
    ...
    async def show_my_wives_command(self, event: AstrMessageEvent):
        """显示用户的抽取历史，别名"抽取历史" - @版本"""
        async for result in self._show_my_wives_common(event):
            yield result
    
    @filter.message_text("我的老婆", alias={'抽取历史'})
    async def show_my_wives(self, event: AstrMessageEvent):
        """显示用户的抽取历史，别名"抽取历史" - 文本版本"""
        async for result in self._show_my_wives_common(event):
            yield result
    
    # 添加新的触发方式 - 直接文本匹配
    @filter.regex(r'^看(看)?(我)?(的)?老婆$', regex_flags=0)
    async def show_my_wives_regex(self, event: AstrMessageEvent):
        """看看我的老婆 - 正则匹配"""
        async for result in self._show_my_wives_common(event):
            yield result
    ...
    @filter.command("重置记录")
    async def reset_records_command(self, event: AstrMessageEvent):
        """仅bot管理员可使用：重置所有群聊的所有记录（慎用！）- @版本"""
        self._reset_daily_records()
        yield event.plain_result("今日抽取记录已重置！")
    
    @filter.permission_type(filter.PermissionType.ADMIN)
    @filter.message_text("重置记录")
    async def reset_records(self, event: AstrMessageEvent):
        """仅bot管理员可使用：重置所有群聊的所有记录（慎用！）- 文本版本"""
        self._reset_daily_records()
        yield event.plain_result("今日抽取记录已重置！")
    
    async def _show_help_common(self, event: AstrMessageEvent):
        """显示帮助的共用方法"""
        daily_limit = self.config.get("daily_limit", 3)
        excluded_count = len(self.config.get("excluded_users", []))
        help_text = f"""=== 抽老婆帮助 v2.0.7 ===
...
• 自动排除Bot和发起者本人
• 每日0点自动重置记录
• 支持@机器人和不@机器人两种触发方式（v2.0.7新增）
• 支持直接文本匹配，如"抽个老婆"、"看看我的老婆"等（v2.0.7新增）
...
"""
    ...
    async def show_help_command(self, event: AstrMessageEvent):
        """显示帮助，别名"今日老婆帮助" - @版本"""
        async for result in self._show_help_common(event):
            yield result
    
    @filter.message_text("抽老婆帮助" , alias={'今日老婆帮助'})
    ...
    # 添加新的触发方式 - 直接文本匹配
    @filter.regex(r'^抽老婆(怎么用|怎么玩|用法|帮助|怎么抽)$', regex_flags=0)
    async def show_help_regex(self, event: AstrMessageEvent):
        """抽老婆怎么用 - 正则匹配"""
        async for result in self._show_help_common(event):
            yield result
